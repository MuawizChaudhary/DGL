...
from utils import DataParallelSpecial, loss_calc, to_one_hot, validate
...
def optim_init(ncnn, model, args):
    ...
    for n in range(ncnn):
        to_train = itertools.chain(model.module.main_cnn.blocks[n].parameters(), model.module.auxillary_nets[n].parameters())
...
# change this
    ...
    ncnn = len(model.module.main_cnn.blocks)
    optimizers = optim_init(ncnn, model, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for n in range(ncnn):
            if optimizers[n] is not None:
                top1test = validate(val_loader, model, epoch, n, args.loss_sup, True)
...
def train(train_loader, model, criterion, optimizers, epoch, args):
    ...
    ncnn = len(model.module.main_cnn.blocks)
    losses = [AverageMeter('Loss', ':.4e') for _ in range(ncnn)]
    top1 = [AverageMeter('Acc@1', ':6.2f') for _ in range(ncnn)]
    top5 = [AverageMeter('Acc@5', ':6.2f') for _ in range(ncnn)]
    progress = [ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses[i], top1[i], top5[i]],
        prefix="Epoch: [{}]".format(epoch)) for i in range(ncnn)]
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        for n in range(0, ncnn):
            ...
            output, rep = model(rep, n=n)
            #loss = criterion(output, target)

            # measure accuracy and record loss
            if optimizer is not None:
                loss = loss_calc(output, target, targets_onehot,
                       model.module.main_cnn.blocks[n], args.loss_sup, args.beta, args.no_similarity_std)
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
                rep.detach_()
                ...
                losses[n].update(loss.item(), images.size(0))
                top1[n].update(acc1[0], images.size(0))
                top5[n].update(acc5[0], images.size(0))
